// Google Workspace : Agenda (Calendar) + Gmail (drafts). Token via NextAuth (provider Google) côté web/.
...
// "acme.fr" -> "Acme" (fallback lisible quand on n'a pas mieux).
...
export async function getGoogleMeetings(accessToken: string, fromISO: string, toISO: string): Promise<PhysicalMeeting[]> {
  ...
  return events
    .filter((e) => e.location && !e.hangoutLink && !e.conferenceData) // physique = adresse + pas de visio
    .map((e) => {
...
// Encodage MIME "encoded-word" du sujet (accents FR) + base64url du message RFC822.
